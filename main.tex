...
\newcommand{\cvLink}{https://drive.google.com/file/d/1svoplivwk5Zm-CJOPcnqzunVgKq5JMFW/view?usp=sharing}
\newcommand{\websiteLink}{https://stefanheng.github.io}
\newcommand{\transcriptGtLink}{https://drive.google.com/file/d/10CTg1kPlQiTQWC8n9qOEkd0_n8flor2U/view?usp=sharing}
\newcommand{\transcriptUmlink}{https://drive.google.com/file/d/1cXDQv8PgRQ2GGUN9pupWWdQcZ0xIVtvi/view?usp=sharing}
\newcommand{\transcriptUmdlink}{https://drive.google.com/file/d/1L75BLD-GpzKqhYnGOUk0VjGjNUoivliI/view?usp=sharing}

\begin{filecontents*}{\jobname.xmpdata}
  \Title{Yuzhao (Stefan) Heng — Resume}
  \Author{Yuzhao (Stefan) Heng}
  \Language{en-US}
  \Keywords{NLP; LLM; Machine Learning; Georgia Tech}
\end{filecontents*}  
% ====================================== End of Added ======================================
...
\iftutex
  ...
  \setsansfont{Lato}  % narrowest font which can fit more characters
...
%   }%
% }
% \newcommand{\linkedul}[2]{%
%   \BeginAccSupp{method=escape,ActualText={\detokenize{#1 (#2)}}}%
%     \textcolor{link}{\href{#2}{\ul{#1}}}%
%   \EndAccSupp{}%
% }
% \newcommand{\linked}[2]{%
%   \BeginAccSupp{method=escape,ActualText={\detokenize{#1 (#2)}}}%
...
%   \EndAccSupp{}%
% }
\newcommand{\linked}[2]{\href{#2}{#1}} % 无下划线
\newcommand{\linkedUl}[2]{\textcolor{link}{\href{#2}{\ul{#1}}}} % 有下划线
\newcommand{\TranscriptLink}[2]{\href{#2}{\ul{\textcolor{link}{#1}}}~(\nolinkurl{#2})}
% 用法： \TranscriptLink{Transcript}{https://...}


\usepackage{calc}
% use `enumitem` to add indentation & reduce top & bottom spacing; note * for appending configs for some defined in altacv
\setlist*{before=\vspace{0pt},after=\vspace{0pt},itemsep=0.25ex,topsep=0pt,partopsep=0pt,leftmargin=0.5cm}
...
\iftutex
  ...
  \newcommand{\ltseries}{\addfontfeatures{UprightFont={* Light},ItalicFont={* Light Italic}}}
\else
  \newcommand{\ltseries}{\fontseries{l}\selectfont}
\fi
...
\pdfstringdefDisableCommands{%
  ...
  \def\faMapMarker*{}%
  \def\faCalendar{}%
  \def\faCalendar*[regular]{}%
  \def\faCircle{}%
  \def\faOrcid{}%
}

% convert fontawesome5 icons to pdf s.t. pdf-miner parsing cannot see them & add as noise
% \newcommand{\faImgPdf}[2][1.05em]{%
  % \raisebox{-0.12em}{\includegraphics[height=#1]{assets/icons/#2.pdf}}%
% }
\newcommand{\faIconHeightDefault}{1.05em}
\newcommand{\faIconRaiseDefault}{-0.12em}
% \faImgPdf[<height>][<raise>]{<name-without-.pdf>}
\NewDocumentCommand{\faImgPdf}{ O{\faIconHeightDefault} O{\faIconRaiseDefault} m }{%
  \raisebox{#2}{\includegraphics[height=#1]{assets/icons/#3.pdf}}%
}
...
\fancypagestyle{nontitlepage}{%
    ...
    \fancyfoot[L]{%
        \ifnum\value{page}=\getpagerefnumber{LastPage}%
            ...
            \iftoggle{compact}{
              ~~~See my full \linked{CV}{\cvLink} for the complete record.
            }{
                %
            }
        \fi%
...
\newcommand{\dropFaiconText}[1]{%
  \BeginAccSupp{ActualText={}}#1\EndAccSupp{}%
}


\newcommand{\smallHeader}[1]{{\color{black}{\textbf{#1}}}}
...
\defbibenvironment{bibliography}
  {
    \vspace*{0.65em}% add space above the bib list for the custom header
    \list
      {
        % the label
...
\iftoggle{compact}{%
  \vspace{-0.5em}
  % career summary
  New Grad ML Engineer (LLMs/NLP) focused on post-training, serving, and eval. 
  Scaled vLLM serving on multi-GPU setups (4X throughput; 20K+ Jira issues triaged via RAG/tool-use at Honeywell). 
  Trained a 100M domain LM with data synthesis and hallucination evaluation (-4X) at 22 Minds.
  Stack: PyTorch, HF, vLLM, FastAPI, Docker. 
  M.S. CS @ Georgia Tech (Dec 2025), U.S. Permanent Resident.
  \vspace{0.5ex}
}{
  %
}
...
\input{sections/other-interests.tex}
...
%     % Earlier roles and a fuller skills breakdown are available in my \linked{CV}{\cvLink}.
%     % Feel free to see my full CV which provides the complete record. 
%     See my full \linked{CV}{\cvLink} for the complete record.
% }{
%     %
% }
